...
function ButtonStatusIcon({ status }: { status: ButtonStatus }) {
  ...
  if (status === "error") {
    return <XIcon aria-hidden="true" className="text-destructive" />;
  }
...
const Button = (props: ButtonProps) => {
  ...
  const status = get(props, "status", "idle");
  const disabled = get(props, "disabled");
  ...
  const isIconOnly = startsWith(size ?? "", "icon");
  ...
  return (
    ...
    >
      {status !== "idle" && <ButtonStatusIcon status={status} />}
      {(!isIconOnly || status === "idle") && children}
    </ButtonPrimitive>
  );
};
