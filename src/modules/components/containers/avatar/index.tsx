...
import { avatarExamples, type AvatarExample } from "./examples.tsx";

const AvatarPage = () => {
  const [selectedExample, setSelectedExample] = useState<AvatarExample | null>(
    null,
  );
  const [copiedExampleId, setCopiedExampleId] = useState<string | null>(null);
  const copiedTimer = useRef<ReturnType<typeof setTimeout> | null>(null);

  const copyExample = async (example: AvatarExample) => {
      ...
      <main className="mx-auto w-full max-w-[1524px] px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
            <div className="max-w-4xl">
              ...
              <p className="mt-4 text-base leading-7 text-muted-foreground sm:text-lg">
                Browse reusable avatar patterns for profiles, team rosters, and
                presence indicators. These examples use the Shomaui avatar
                primitives and stay aligned with the existing shadcn design
                system.
              </p>
            </div>
            <Badge className="shrink-0 gap-2 px-3 py-1.5 text-sm">
              <Blocks data-icon="inline-start" />
              {collectionSize(avatarExamples)} examples
            </Badge>
          </div>
        </header>

        <div className="flex flex-col gap-8">
          {map(avatarExamples, (example) => {
        ...
        {selectedExample && (
          <CodeSheet
            ...
            title={get(selectedExample, "title", "Avatar")}
            registrySlug={get(selectedExample, "slug", "avatar")}
