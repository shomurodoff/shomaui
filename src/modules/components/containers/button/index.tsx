...
import { buttonExamples, type ButtonExample } from "./examples.tsx";

const ButtonPage = () => {
  const [selectedExample, setSelectedExample] = useState<ButtonExample | null>(
    null,
  );
  const [copiedExampleId, setCopiedExampleId] = useState<string | null>(null);
  const copiedTimer = useRef<ReturnType<typeof setTimeout> | null>(null);

  const copyExample = async (example: ButtonExample) => {
      ...
      <main className="mx-auto w-full max-w-[1524px] px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
            <div className="max-w-4xl">
              ...
              <p className="mt-4 text-base leading-7 text-muted-foreground sm:text-lg">
                Browse reusable button patterns for primary actions, compact
                controls, icon buttons, and async status feedback. Every example
                uses the Shomaui button component with shadcn styling.
              </p>
            </div>
            <Badge className="shrink-0 gap-2 px-3 py-1.5 text-sm">
              <Blocks data-icon="inline-start" />
              {collectionSize(buttonExamples)} examples
            </Badge>
          </div>
        </header>

        <div className="flex flex-col gap-8">
          {map(buttonExamples, (example) => {
        ...
        {selectedExample && (
          <CodeSheet
            ...
            title={get(selectedExample, "title", "Button")}
            registrySlug={get(selectedExample, "slug", "button")}
